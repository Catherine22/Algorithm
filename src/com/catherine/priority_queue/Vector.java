...
public class Vector<E> {
	 ...
	 * 加倍式扩容虽然牺牲内存空间，空间利用率(已使用/全部空间)至少大于50%，但在运行速度上远超越递增式扩容。<br>
	 * 递增式扩容须耗时O(1) ，空间利用率(已使用／全部空间)=100%<br>
	 * 结论：加倍式扩容优于递增式扩容
	 * 
	 */
	 ...
	 * @return new array
	 */
	public E[] insert(E[] array, int position, E value) {
		E[] result = doubleArray(array);
	 ...
	 * @return new array
	 */
	public E[] remove(E[] array, int fromPos, int toPos) {
		int pointer = fromPos;
	 ...
	 * @return new array
	 */
	public E[] remove(E[] array, int pos) {
	 ...
	 */
	public int find(E[] array, E value) {
	 ...
	 * @return new array
	 */
	public E[] removeDuplicates(E[] array) {
		long start = System.currentTimeMillis();
		// int[] newArray = null;
		int comparePos = 1;
		while (comparePos < array.length) {
			...
			if (prefixPos != -1 && array[prefixPos] == array[comparePos] && array[comparePos] != null)
	 ...
	 */
	@SuppressWarnings("unchecked")
	private E[] shrink(E[] array) {
		int nullNum = 0;
		// 用Math.ceil无条件进位，用（float）保证运算结果不会被去小数点
		temp = new Object[(int) Math.ceil((float) array.length / 2)];
		for (int j = 0; j < temp.length; j++) {
			temp[j] = array[j];
	 ...
	 * @return position (-1 as not found)
	 */
	private int find(E[] array, int fromPos, int toPos, E value) {
		int pointer = toPos + 1;
		// 停止条件 1. 扫描范围超出合法位置（由后往前检查） 2. 找到value
